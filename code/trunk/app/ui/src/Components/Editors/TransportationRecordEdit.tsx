...
import { emptyTransportationRecord, transportationRecordToChoiceArgs, validateTransportationRecord, TransportationRecord } from "../../Types/TransportationRecord";
...
import { TransportationRecordEditHeader } from "../DataDisplay/EditHeaders/TransportationRecordEditHeader";
...
export class TransportationRecordEdit extends React.Component<TransportationRecordEditProps, TransportationRecordEditState> {
  ...
  private putToLedger = () => {
    ...
    ledger.exerciseByKey(Product.AddTransportationRecordToProduct, productKey, choiceArgs)
      .then(() => this.onClose());
  }
  ...
  private onOkay = () => {
    const { record } = this.state;
    const { product } = this.props;
    if (validateTransportationRecord(record, product)) this.putToLedger();
  ...
  render() {
    return (
      <InputDialog
        ...
        onEnter={() => this.setState({ record: emptyTransportationRecord(this.props.product), validationError: false })}
      />
    );
  }
}
