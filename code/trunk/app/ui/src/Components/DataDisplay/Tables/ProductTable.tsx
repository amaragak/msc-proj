...
export interface ProductTableComponentOwnProps {
  ...
  onReceiveRecords?: (record: Product[]) => void;
}
...
class ProductTableComponent extends React.Component<ProductTableComponentProps, {}> {
  render() {
    const { user, columnNames, recordToRow, onRecordSelected, classes, onReceiveRecords } = this.props;
    return(
      <GenericTable<Product, ProductTemplate>
        ...
        onReceiveRecords={onReceiveRecords}
      />
    );
  }
}
